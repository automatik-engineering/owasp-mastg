...
To test for the use of broken encryption modes in iOS apps, we need to focus on methods from cryptographic frameworks and libraries that are used to configure and apply encryption modes.
...
When `kCCOptionECBMode` is set in the options parameter, the encryption uses ECB mode which is [considered vulnerable](https://support.google.com/faqs/answer/10046138?hl=en). The default behavior (when `kCCOptionECBMode` is not set) is to use CBC mode, which is more secure when used with a proper initialization vector (IV).
