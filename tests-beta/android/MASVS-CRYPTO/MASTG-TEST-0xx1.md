...
This test verifies that asymmetric keys are used for only one clearly defined purpose, as required by ["NIST.SP.800-57pt1r5"](https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-57pt1r5.pdf).
...
The output should contain a list of locations where asymmetric keys are created, along with backtraces showing each `KeyPair` instantiation along with its purposes.
...
In other words, ensure that the app doesn't create a key that combines purposes across roles (for example, signing **and** decrypting).
